...
(function ($) {
    ...
    function SetDotRating(element, value) {
        ...
        if(element.data("dot-color-empty")) {
            ...
            dotlist.children(".ss-dot:not(.ss-dot-marked)").css("background-color",
            element.data("dot-color-empty"));
        }

        if(element.data("dot-color-marked")) {
            // Set the color to all dots who are marked
            dotlist.children(".ss-dot.ss-dot-marked").slice(0, value).css("background-color",
            element.data("dot-color-marked"));
        }
    }

    function SetDotRatingLeft(element, value) {
        // dotlist is the inner div that contains all dots
        var dotlist = element.children(".ss-dots-left");
        dotlist.children(".ss-dot").removeClass("ss-dot-marked");
    ...
    function InitDotRatingLeft(element) {
        var max, value;
        max = element.data("dot-max");
        value = element.data("dot-value");

        element.html("<div class='ss-title'></div><div class='ss-dots-left'></div>");
        element.children(".ss-title").html(element.data("dot-title"));
    ...
    $(document).ready(function () {
        ...
        $("ss-rating-left").each(function(){
            InitDotRatingLeft($(this));
        });
    });
